...
def Combine_TMMC_lnpi(file, file_dir, start, sims, p0):
    ...
    if(path.exists(TMMCFile)):
        InitData = pd.read_table(TMMCFile, delim_whitespace=True, 
                                 skiprows = 4)#, skipfooter = end)
        ...
        for sim in range(start + 1, sims):
            ...
            if(path.exists(TMMCFile)):
                Dataset = pd.read_table(TMMCFile, delim_whitespace=True, 
                                        skiprows = 4)#, skipfooter = end)
...
TMMCfile = 'TMMC/TMMC_Statistics.data'
...
MinMaxData.to_csv("Combined_MinMax-lnpi.csv")
