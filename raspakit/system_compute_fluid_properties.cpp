...
void System::computeComponentFluidProperties()
{
    ...
    double pressure = simulationBox.pressure * Units::PressureConversionFactor;
    double temperature = simulationBox.temperature;
    ...
    for (size_t i = 0; i < components.size(); ++i)
    {
        if (components[i].swapable)
        {
            ...
            case EquationOfState::SoaveRedlichKwong:
                ...
                break;
            case EquationOfState::PengRobinsonGasem:
                ...
                B[i] = b[i] * pressure / temp;
                break;
            case EquationOfState::PengRobinson:
            ...
            if (components[i].swapable)
            {
                for (size_t j = 0; j < static_cast<size_t>(numberOfSolutions); ++j)
                {
                    ...
                    fugacityCoefficients[i][j] = std::exp((B[i] / Bmix) * (compressibility[j] - 1.0) - std::log(compressibility[j] - Bmix)
                        ...
                        log((compressibility[j] + (1.0 + std::sqrt(2.0)) * Bmix) / (compressibility[j] + (1.0 - std::sqrt(2)) * Bmix)));
                }
            }
        }
        break;
