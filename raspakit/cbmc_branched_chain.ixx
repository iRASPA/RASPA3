...
struct FirstBeadData
{
    ...
    FirstBeadData(Atom atom, RunningEnergy energies, double logRosenbluthWeight, double storedR) :
...
export struct ChainData
{
    ...
    ChainData(std::vector<Atom> atom, RunningEnergy energies, double logRosenbluthWeight, double storedR) :
        atom(atom), energies(energies), logRosenbluthWeight(logRosenbluthWeight), storedR(storedR)
    {
    }
};
...
export struct CBMCLinearChain
{
    ...
    std::vector<Atom> filterNonOverlappingTrialPositions(std::vector <std::pair<Atom, RunningEnergy>> trialPositions) const noexcept;
