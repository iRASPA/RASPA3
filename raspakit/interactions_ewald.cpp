...
//
void Ewald::registerEwaldFourierEnergySingleIon(const ForceField &forceField, 
                                                ...
                                                double3 position, double charge)
{
  ...
  CoulombicFourierEnergySingleIon = -Units::CoulombicConversionFactor * 
                                     ((2.0 * std::numbers::pi / simulationBox.volume) * energy_sum
                                     - alpha/std::sqrt(std::numbers::pi));
}


void Ewald::computeEwaldFourierRigidEnergy(const ForceField &forceField, const SimulationBox &simulationBox,
                                           std::span<const Atom> frameworkAtoms, RunningEnergy& energyStatus)
{
  ...
  size_t numberOfAtoms = frameworkAtoms.size();
  ...
  for (size_t i = 0; i != numberOfAtoms; ++i)
  {
    ...
    double3 s = 2.0 * std::numbers::pi * (inv_box * frameworkAtoms[i].position);
      ...
      for (std::make_signed_t<std::size_t> kz = -kz_max; kz <= kz_max; ++kz)
      {
        ...
        if ((kx * kx + ky * ky + kz * kz) != 0)
        {
          ...
          for (size_t i = 0; i != numberOfAtoms; ++i)
          {
            ...
            eikz_temp.imag(kz >= 0 ? eikz_temp.imag() : -eikz_temp.imag());
            double charge = frameworkAtoms[i].charge;
            double scaling = frameworkAtoms[i].scalingCoulomb;
...
void Ewald::computeEwaldFourierEnergy(const ForceField &forceField, const SimulationBox &simulationBox,            
...
// compute gradient
ForceFactor Ewald::computeEwaldFourierGradient(const ForceField &forceField, const SimulationBox &simulationBox,
...
RunningEnergy Ewald::energyDifferenceEwaldFourier(const ForceField &forceField, const SimulationBox &simulationBox,
                             std::vector<std::pair<std::complex<double>, std::complex<double>>> &storedWavevectors, 
