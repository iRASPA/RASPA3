...
import units;
import threadpool;
import energy_interactions_framework_molecule;
...
[[nodiscard]] std::optional<EnergyStatus> System::computeFrameworkSpanMoleculeEnergy(std::span<const Atom>::iterator startIterator, std::span<const Atom>::iterator endIterator, std::span<Atom> atoms, std::make_signed_t<std::size_t> skip) const
{
	double3 dr, s, t;
	double rr;
...
[[nodiscard]] std::optional<EnergyStatus> System::computeFrameworkMoleculeEnergy(std::span<Atom> atoms, std::make_signed_t<std::size_t> skip) const noexcept
{
  ...
  for(size_t i = 0 ; i < numberOfHelperThreads; ++i)
  {
      ...
      threads[i] = pool.submit(std::bind(&System::computeFrameworkSpanMoleculeEnergy, this, block_start, block_end, atoms, skip));
      block_start=block_end;
  }
  std::optional<EnergyStatus> energy = computeFrameworkSpanMoleculeEnergy(block_start, frameworkAtoms.end(), atoms, skip);
