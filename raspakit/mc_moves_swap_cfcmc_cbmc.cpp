         ...
         bool insertionDisabled, bool deletionDisabled)
{
  ...
  double maxChange = system.components[selectedComponent].mc_moves_probabilities.statistics_SwapMove_CFCMC_CBMC.maxChange[2];
  ...
  if (selectedNewBin >= std::make_signed_t<std::size_t>(lambda.numberOfBins)) // Insertion move
  {
    ...
    system.components[selectedComponent].mc_moves_probabilities.statistics_SwapMove_CFCMC_CBMC.totalCounts[0] += 1;
    ...
    RunningEnergy tailEnergyDifference = system.computeInterMolecularTailEnergyDifference(fractionalMolecule, oldFractionalMolecule) +
                                         system.computeFrameworkMoleculeTailEnergyDifference(fractionalMolecule, oldFractionalMolecule);
    ...
    std::optional<ChainData> growData = system.growMoleculeSwapInsertion(random, growType, cutOffVDW, cutOffCoulomb, selectedComponent, newMolecule, newLambda, newatoms);
    ...
    system.components[selectedComponent].mc_moves_probabilities.statistics_SwapMove_CFCMC_CBMC.totalConstructed[0] += 1;
    ...
    RunningEnergy tailEnergyDifferenceGrow = system.computeInterMolecularTailEnergyDifference(std::span(growData->atom.begin(), growData->atom.end()), {}) +
                                         system.computeFrameworkMoleculeTailEnergyDifference(std::span(growData->atom.begin(), growData->atom.end()), {});

    double correctionFactorEwald = std::exp(-system.beta * (energyFourierDifference.total() + tailEnergyDifferenceGrow.total()));
    ...
    double Pacc = preFactor * (growData->RosenbluthWeight / idealGasRosenbluthWeight) * exp(-system.beta * energyDifference.total() + biasTerm);
    ...
    if (random.uniform() < biasTransitionMatrix * Pacc)
    {
      ...
      system.components[selectedComponent].mc_moves_probabilities.statistics_SwapMove_CFCMC_CBMC.totalAccepted[0] += 1;

      return {energyDifference + growData->energies + energyFourierDifference + tailEnergyDifferenceGrow, double3(0.0, 1.0 - Pacc, Pacc)};
    };
  ...
  else if (selectedNewBin < 0) // Deletion move
  {
    ...
    if (system.numberOfIntegerMoleculesPerComponent[selectedComponent] > 0)
    {
      ...
      system.components[selectedComponent].mc_moves_probabilities.statistics_SwapMove_CFCMC_CBMC.totalCounts[1] += 1;
      ...
      // (1) Biased: the existing fractional molecule is retraced using CBMC with lambda=lambda_o, fractional molecule is removed.
      std::chrono::system_clock::time_point t1 = std::chrono::system_clock::now();
      ChainData retraceData = system.retraceMoleculeSwapDeletion(random, cutOffVDW, cutOffCoulomb, selectedComponent, indexFractionalMolecule, fractionalMolecule, oldLambda, 0.0);            
      std::chrono::system_clock::time_point t2 = std::chrono::system_clock::now();
      ...
      RunningEnergy tailEnergyDifferenceRetrace = system.computeInterMolecularTailEnergyDifference({}, fractionalMolecule) +
                                                  system.computeFrameworkMoleculeTailEnergyDifference({}, fractionalMolecule);
      
      double correctionFactorEwald = std::exp(-system.beta * (energyFourierDifference.total() + tailEnergyDifferenceRetrace.total()));
      ...
      RunningEnergy tailEnergyDifference = system.computeInterMolecularTailEnergyDifference(newFractionalMolecule, savedFractionalMolecule) +
                                           system.computeFrameworkMoleculeTailEnergyDifference(newFractionalMolecule, savedFractionalMolecule);
      RunningEnergy energyDifference = frameworkDifference.value() + moleculeDifference.value() + EwaldEnergyDifference + tailEnergyDifference;
      
      system.components[selectedComponent].mc_moves_probabilities.statistics_SwapMove_CFCMC_CBMC.constructed[1] += 1;
      system.components[selectedComponent].mc_moves_probabilities.statistics_SwapMove_CFCMC_CBMC.totalConstructed[1] += 1;
      ...
      double biasTerm = lambda.biasFactor[newBin] - lambda.biasFactor[oldBin];
      double Pacc = preFactor * (idealGasRosenbluthWeight / retraceData.RosenbluthWeight) * exp(-system.beta * energyDifference.total() + biasTerm);
      ...
      if (random.uniform() < biasTransitionMatrix * Pacc)
      {
        ...
        system.components[selectedComponent].mc_moves_probabilities.statistics_SwapMove_CFCMC_CBMC.totalAccepted[1] += 1;
      
        return {energyDifference + energyFourierDifference + tailEnergyDifferenceRetrace - retraceData.energies, double3(Pacc, 1.0 - Pacc, 0.0)};
      };
  ...
  else // Lambda-move
  {
    ...
    system.components[selectedComponent].mc_moves_probabilities.statistics_SwapMove_CFCMC_CBMC.totalCounts[2] += 1;
    ...
    std::chrono::system_clock::time_point w1 = std::chrono::system_clock::now();
    [[maybe_unused]] RunningEnergy tailEnergyDifference = system.computeInterMolecularTailEnergyDifference(trialPositions, molecule) +
    ...
    system.components[selectedComponent].mc_moves_probabilities.statistics_SwapMove_CFCMC_CBMC.totalConstructed[2] += 1;
    ...
    if (random.uniform() < std::exp(-system.beta * energyDifference.total() + biasTerm))
    {
      ...
      system.components[selectedComponent].mc_moves_probabilities.statistics_SwapMove_CFCMC_CBMC.totalAccepted[2] += 1;
