...
TEST(hessian_inter_lennard_jones, Test_gradient_cartesian_methane_in_CHA_triclinic_1x1x1)
{
...

  {
    ...
    auto [_, reference_cartesian, _] = Interactions::calculateHessianAtPositionVDW(system.forceField, system.simulationBox, posB, typeB, frameworkAtoms);
    ...
    auto [x2_energy, _, _] = Interactions::calculateHessianAtPositionVDW(system.forceField, system.simulationBox, posB, typeB, frameworkAtoms);

    posB = system.simulationBox.cell * double3(s.x - 0.5 * delta, s.y, s.z);
    auto [x1_energy, _, _] = Interactions::calculateHessianAtPositionVDW(system.forceField, system.simulationBox, posB, typeB, frameworkAtoms);

    // finite difference y
    posB = system.simulationBox.cell * double3(s.x, s.y + 0.5 * delta, s.z);
    auto [y2_energy, _, _] = Interactions::calculateHessianAtPositionVDW(system.forceField, system.simulationBox, posB, typeB, frameworkAtoms);

    posB = system.simulationBox.cell * double3(s.x, s.y - 0.5 * delta, s.z);
    auto [y1_energy, _, _] = Interactions::calculateHessianAtPositionVDW(system.forceField, system.simulationBox, posB, typeB, frameworkAtoms);

    // finite difference z
    posB = system.simulationBox.cell * double3(s.x, s.y, s.z + 0.5 * delta);
    auto [z2_energy, _, _] = Interactions::calculateHessianAtPositionVDW(system.forceField, system.simulationBox, posB, typeB, frameworkAtoms);

    posB = system.simulationBox.cell * double3(s.x, s.y, s.z - 0.5 * delta);
    auto [z1_energy, _, _] = Interactions::calculateHessianAtPositionVDW(system.forceField, system.simulationBox, posB, typeB, frameworkAtoms);
...
TEST(hessian_inter_lennard_jones, Test_hessian_cartesian_methane_in_CHA_triclinic_1x1x1)
{
...

  {
    ...
    auto [_, _, reference_cartesian] = Interactions::calculateHessianAtPositionVDW(system.forceField, system.simulationBox, posB_reference, typeB, frameworkAtoms);

    // finite difference x
    posB = double3(posB_reference.x + 0.5 * delta, posB_reference.y, posB_reference.z);
    auto [_, x2_gradient, _] = Interactions::calculateHessianAtPositionVDW(system.forceField, system.simulationBox, posB, typeB, frameworkAtoms);

    posB = double3(posB_reference.x - 0.5 * delta, posB_reference.y, posB_reference.z);
    auto [_, x1_gradient, _] = Interactions::calculateHessianAtPositionVDW(system.forceField, system.simulationBox, posB, typeB, frameworkAtoms);

    // finite difference y
    posB = double3(posB_reference.x, posB_reference.y + 0.5 * delta, posB_reference.z);
    auto [_, y2_gradient, _] = Interactions::calculateHessianAtPositionVDW(system.forceField, system.simulationBox, posB, typeB, frameworkAtoms);

    posB = double3(posB_reference.x, posB_reference.y - 0.5 * delta, posB_reference.z);
    auto [_, y1_gradient, _] = Interactions::calculateHessianAtPositionVDW(system.forceField, system.simulationBox, posB, typeB, frameworkAtoms);

    // finite difference z
    posB = double3(posB_reference.x, posB_reference.y, posB_reference.z + 0.5 * delta);
    auto [_, z2_gradient, _] = Interactions::calculateHessianAtPositionVDW(system.forceField, system.simulationBox, posB, typeB, frameworkAtoms);

    posB = double3(posB_reference.x, posB_reference.y, posB_reference.z - 0.5 * delta);
    auto [_, z1_gradient, _] = Interactions::calculateHessianAtPositionVDW(system.forceField, system.simulationBox, posB, typeB, frameworkAtoms);
