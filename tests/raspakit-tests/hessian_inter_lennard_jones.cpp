...
TEST(hessian_inter_lennard_jones, Test_gradient_cartesian_methane_in_CHA_triclinic_1x1x1)
{
...

  {
    ...
    auto [e, reference_cartesian, d2] = Interactions::calculateHessianAtPositionVDW(
        system.forceField, system.simulationBox, posB, typeB, frameworkAtoms);
    ...
    auto [x2_energy, d1x2, d2x2] = Interactions::calculateHessianAtPositionVDW(system.forceField, system.simulationBox,
                                                                               posB, typeB, frameworkAtoms);

    posB = system.simulationBox.cell * double3(s.x - 0.5 * delta, s.y, s.z);
    auto [x1_energy, d1x1, d2x1] = Interactions::calculateHessianAtPositionVDW(system.forceField, system.simulationBox,
                                                                               posB, typeB, frameworkAtoms);

    // finite difference y
    posB = system.simulationBox.cell * double3(s.x, s.y + 0.5 * delta, s.z);
    auto [y2_energy, d1y2, d2y2] = Interactions::calculateHessianAtPositionVDW(system.forceField, system.simulationBox,
                                                                               posB, typeB, frameworkAtoms);

    posB = system.simulationBox.cell * double3(s.x, s.y - 0.5 * delta, s.z);
    auto [y1_energy, d1y1, d2y1] = Interactions::calculateHessianAtPositionVDW(system.forceField, system.simulationBox,
                                                                               posB, typeB, frameworkAtoms);

    // finite difference z
    posB = system.simulationBox.cell * double3(s.x, s.y, s.z + 0.5 * delta);
    auto [z2_energy, d1z2, d2z2] = Interactions::calculateHessianAtPositionVDW(system.forceField, system.simulationBox,
                                                                               posB, typeB, frameworkAtoms);

    posB = system.simulationBox.cell * double3(s.x, s.y, s.z - 0.5 * delta);
    auto [z1_energy, d1z1, d2z1] = Interactions::calculateHessianAtPositionVDW(system.forceField, system.simulationBox,
                                                                               posB, typeB, frameworkAtoms);
...
TEST(hessian_inter_lennard_jones, Test_hessian_cartesian_methane_in_CHA_triclinic_1x1x1)
{
...

  {
    ...
    auto [e, d1, reference_cartesian] = Interactions::calculateHessianAtPositionVDW(
        system.forceField, system.simulationBox, posB_reference, typeB, frameworkAtoms);

    // finite difference x
    posB = double3(posB_reference.x + 0.5 * delta, posB_reference.y, posB_reference.z);
    auto [ex2, x2_gradient, d2x2] = Interactions::calculateHessianAtPositionVDW(system.forceField, system.simulationBox,
                                                                                posB, typeB, frameworkAtoms);

    posB = double3(posB_reference.x - 0.5 * delta, posB_reference.y, posB_reference.z);
    auto [ex1, x1_gradient, d2x1] = Interactions::calculateHessianAtPositionVDW(system.forceField, system.simulationBox,
                                                                                posB, typeB, frameworkAtoms);

    // finite difference y
    posB = double3(posB_reference.x, posB_reference.y + 0.5 * delta, posB_reference.z);
    auto [ey2, y2_gradient, d2y2] = Interactions::calculateHessianAtPositionVDW(system.forceField, system.simulationBox,
                                                                                posB, typeB, frameworkAtoms);

    posB = double3(posB_reference.x, posB_reference.y - 0.5 * delta, posB_reference.z);
    auto [ey1, y1_gradient, d2y1] = Interactions::calculateHessianAtPositionVDW(system.forceField, system.simulationBox,
                                                                                posB, typeB, frameworkAtoms);

    // finite difference z
    posB = double3(posB_reference.x, posB_reference.y, posB_reference.z + 0.5 * delta);
    auto [ez2, z2_gradient, d2z2] = Interactions::calculateHessianAtPositionVDW(system.forceField, system.simulationBox,
                                                                                posB, typeB, frameworkAtoms);

    posB = double3(posB_reference.x, posB_reference.y, posB_reference.z - 0.5 * delta);
    auto [ez1, z1_gradient, d2z1] = Interactions::calculateHessianAtPositionVDW(system.forceField, system.simulationBox,
                                                                                posB, typeB, frameworkAtoms);
