...
TEST(energy_decomposition, CO2_Methane_in_Box)
{
  ...
  std::pair<EnergyStatus, double3x3> strainDerivative =
    Interactions::computeInterMolecularEnergyStrainDerivative(system.forceField, system.components, 
                                                              system.simulationBox, system.atomPositions);
  strainDerivative.first.sumTotal();

  EXPECT_NEAR(energy.moleculeMoleculeVDW + energy.moleculeMoleculeCharge, strainDerivative.first.totalEnergy.energy ,1e-6);
}
...
TEST(energy_decomposition, CO2_Methane_in_Box_Ewald)
{
  ...
  Component co2_2 = Component(2, forceField, "CO2", 304.1282, 7377300.0, 0.22394,
    { // double3 position, double charge, double lambda, uint16_t type, uint8_t componentId, uint32_t moleculeId
      Atom(double3(0.0, 0.0,  1.149), -0.3256, 1.0, 0, 4, 1, 0),
      Atom(double3(0.0, 0.0,  0.0  ),  0.6512, 1.0, 0, 3, 1, 0),
      Atom(double3(0.0, 0.0, -1.149), -0.3256, 1.0, 0, 4, 1, 0)
  ...
  std::pair<EnergyStatus, double3x3> strainDerivative =
    Interactions::computeEwaldFourierEnergyStrainDerivative(system.eik_x, system.eik_y, system.eik_z, system.eik_xy,
                     ...
                     system.frameworkComponents, system.components, system.numberOfMoleculesPerComponent, system.spanOfMoleculeAtoms());
