...
TEST(third_derivative_inter_lennard_jones, Test_gradient_cartesian_methane_in_CHA_triclinic_1x1x1)
{
...

  {
    ...
    auto [e, reference_cartesian, d1, d2] = Interactions::calculateTricubicDerivativeAtPosition(ForceField::InterpolationGridType::LennardJones, system.forceField, system.simulationBox, posB, typeB, frameworkAtoms);
    ...
    auto [x2_energy, d1x2, d2x2, d3x2] = Interactions::calculateTricubicDerivativeAtPosition(ForceField::InterpolationGridType::LennardJones, system.forceField, system.simulationBox, posB, typeB, frameworkAtoms);

    posB = system.simulationBox.cell * double3(s.x - 0.5 * delta, s.y, s.z);
    auto [x1_energy, d1x1, d2x1, d3x1] = Interactions::calculateTricubicDerivativeAtPosition(ForceField::InterpolationGridType::LennardJones, system.forceField, system.simulationBox, posB, typeB, frameworkAtoms);

    // finite difference y
    posB = system.simulationBox.cell * double3(s.x, s.y + 0.5 * delta, s.z);
    auto [y2_energy, d1y2, d2y2, d3y2] = Interactions::calculateTricubicDerivativeAtPosition(ForceField::InterpolationGridType::LennardJones, system.forceField, system.simulationBox, posB, typeB, frameworkAtoms);

    posB = system.simulationBox.cell * double3(s.x, s.y - 0.5 * delta, s.z);
    auto [y1_energy, d1y1, d2y1, d3y1] = Interactions::calculateTricubicDerivativeAtPosition(ForceField::InterpolationGridType::LennardJones, system.forceField, system.simulationBox, posB, typeB, frameworkAtoms);

    // finite difference z
    posB = system.simulationBox.cell * double3(s.x, s.y, s.z + 0.5 * delta);
    auto [z2_energy, d1z2, d2z2, d3z2] = Interactions::calculateTricubicDerivativeAtPosition(ForceField::InterpolationGridType::LennardJones, system.forceField, system.simulationBox, posB, typeB, frameworkAtoms);

    posB = system.simulationBox.cell * double3(s.x, s.y, s.z - 0.5 * delta);
    auto [z1_energy, d1z1, d2z1, d3z1] = Interactions::calculateTricubicDerivativeAtPosition(ForceField::InterpolationGridType::LennardJones, system.forceField, system.simulationBox, posB, typeB, frameworkAtoms);
...
TEST(third_derivative_inter_lennard_jones, Test_hessian_cartesian_methane_in_CHA_triclinic_1x1x1)
{
...

  {
    ...
    auto [e, d1, reference_cartesian, d3] = Interactions::calculateTricubicDerivativeAtPosition(ForceField::InterpolationGridType::LennardJones, system.forceField, system.simulationBox, posB_reference, typeB, frameworkAtoms);

    // finite difference x
    posB = double3(posB_reference.x + 0.5 * delta, posB_reference.y, posB_reference.z);
    auto [ex2, x2_gradient, d2x2, d3x2] = Interactions::calculateTricubicDerivativeAtPosition(ForceField::InterpolationGridType::LennardJones, system.forceField, system.simulationBox, posB, typeB, frameworkAtoms);

    posB = double3(posB_reference.x - 0.5 * delta, posB_reference.y, posB_reference.z);
    auto [ex1, x1_gradient, d2x1, d3x1] = Interactions::calculateTricubicDerivativeAtPosition(ForceField::InterpolationGridType::LennardJones, system.forceField, system.simulationBox, posB, typeB, frameworkAtoms);

    // finite difference y
    posB = double3(posB_reference.x, posB_reference.y + 0.5 * delta, posB_reference.z);
    auto [ey2, y2_gradient, d2y2, d3y2] = Interactions::calculateTricubicDerivativeAtPosition(ForceField::InterpolationGridType::LennardJones, system.forceField, system.simulationBox, posB, typeB, frameworkAtoms);

    posB = double3(posB_reference.x, posB_reference.y - 0.5 * delta, posB_reference.z);
    auto [ey1, y1_gradient, d2y1, d3y1] = Interactions::calculateTricubicDerivativeAtPosition(ForceField::InterpolationGridType::LennardJones, system.forceField, system.simulationBox, posB, typeB, frameworkAtoms);

    // finite difference z
    posB = double3(posB_reference.x, posB_reference.y, posB_reference.z + 0.5 * delta);
    auto [ez2, z2_gradient, d2z2, d3z2] = Interactions::calculateTricubicDerivativeAtPosition(ForceField::InterpolationGridType::LennardJones, system.forceField, system.simulationBox, posB, typeB, frameworkAtoms);

    posB = double3(posB_reference.x, posB_reference.y, posB_reference.z - 0.5 * delta);
    auto [ez1, z1_gradient, d2z1, d3z1] = Interactions::calculateTricubicDerivativeAtPosition(ForceField::InterpolationGridType::LennardJones, system.forceField, system.simulationBox, posB, typeB, frameworkAtoms);
...
TEST(third_derivative_inter_lennard_jones, Test_hessian_fractional_methane_in_CHA_triclinic_1x1x1)
{
  ...
  {
    double3 s = system.simulationBox.inverseCell * posB;
    auto [e, d1, reference_cartesian, d3] = Interactions::calculateTricubicDerivativeAtPosition(ForceField::InterpolationGridType::LennardJones, system.forceField, system.simulationBox, posB, typeB, frameworkAtoms);
    double3x3 reference_fractional = system.simulationBox.cell.transpose() * reference_cartesian * system.simulationBox.cell;

    // finite difference x
    posB = system.simulationBox.cell * double3(s.x + 0.5 * delta, s.y, s.z);
    auto [ex2, x2_gradient, d1x2, d2x2] = Interactions::calculateTricubicDerivativeAtPosition(ForceField::InterpolationGridType::LennardJones, system.forceField, system.simulationBox, posB, typeB, frameworkAtoms);

    posB = system.simulationBox.cell * double3(s.x - 0.5 * delta, s.y, s.z);
    auto [ex1, x1_gradient, d1x1, d2x1] = Interactions::calculateTricubicDerivativeAtPosition(ForceField::InterpolationGridType::LennardJones, system.forceField, system.simulationBox, posB, typeB, frameworkAtoms);

    // finite difference y
    posB = system.simulationBox.cell * double3(s.x, s.y + 0.5 * delta, s.z);
    auto [ey2, y2_gradient, d1y2, d2y2] = Interactions::calculateTricubicDerivativeAtPosition(ForceField::InterpolationGridType::LennardJones, system.forceField, system.simulationBox, posB, typeB, frameworkAtoms);

    posB = system.simulationBox.cell * double3(s.x, s.y - 0.5 * delta, s.z);
    auto [ey1, y1_gradient, d1y1, d2y1] = Interactions::calculateTricubicDerivativeAtPosition(ForceField::InterpolationGridType::LennardJones, system.forceField, system.simulationBox, posB, typeB, frameworkAtoms);

    // finite difference z
    posB = system.simulationBox.cell * double3(s.x, s.y, s.z + 0.5 * delta);
    auto [ez2, z2_gradient, d1z2, d2z2] = Interactions::calculateTricubicDerivativeAtPosition(ForceField::InterpolationGridType::LennardJones, system.forceField, system.simulationBox, posB, typeB, frameworkAtoms);

    posB = system.simulationBox.cell * double3(s.x, s.y, s.z - 0.5 * delta);
    auto [ez1, z1_gradient, d1z1, d2z1] = Interactions::calculateTricubicDerivativeAtPosition(ForceField::InterpolationGridType::LennardJones, system.forceField, system.simulationBox, posB, typeB, frameworkAtoms);
...
TEST(third_derivative_inter_lennard_jones, Test_third_derivative_cartesian_methane_in_CHA_triclinic_1x1x1)
{
...

  {
    ...
    auto [e, d1, d2, reference_cartesian] = Interactions::calculateTricubicDerivativeAtPosition(ForceField::InterpolationGridType::LennardJones, system.forceField, system.simulationBox, posB_reference, typeB, frameworkAtoms);

    posB = double3(posB_reference.x + 0.5 * delta, posB_reference.y, posB_reference.z);
    auto [ex2, d1x2, x2_hessian, d3x2] = Interactions::calculateTricubicDerivativeAtPosition(ForceField::InterpolationGridType::LennardJones, system.forceField, system.simulationBox, posB, typeB, frameworkAtoms);

    posB = double3(posB_reference.x - 0.5 * delta, posB_reference.y, posB_reference.z);
    auto [ex1, d1x1, x1_hessian, d3x1] = Interactions::calculateTricubicDerivativeAtPosition(ForceField::InterpolationGridType::LennardJones, system.forceField, system.simulationBox, posB, typeB, frameworkAtoms);
    ...
    posB = double3(posB_reference.x, posB_reference.y + 0.5 * delta, posB_reference.z);
    auto [ey2, d1y2, y2_hessian, d3y2] = Interactions::calculateTricubicDerivativeAtPosition(ForceField::InterpolationGridType::LennardJones, system.forceField, system.simulationBox, posB, typeB, frameworkAtoms);

    posB = double3(posB_reference.x, posB_reference.y - 0.5 * delta, posB_reference.z);
    auto [ey1, d1y1, y1_hessian, d3y1] = Interactions::calculateTricubicDerivativeAtPosition(ForceField::InterpolationGridType::LennardJones, system.forceField, system.simulationBox, posB, typeB, frameworkAtoms);
    ...
    posB = double3(posB_reference.x, posB_reference.y, posB_reference.z + 0.5 * delta);
    auto [ez2, d1z2, z2_hessian, d3z2] = Interactions::calculateTricubicDerivativeAtPosition(ForceField::InterpolationGridType::LennardJones, system.forceField, system.simulationBox, posB, typeB, frameworkAtoms);

    posB = double3(posB_reference.x, posB_reference.y, posB_reference.z - 0.5 * delta);
    auto [ez1, d1z1, z1_hessian, d3z1] = Interactions::calculateTricubicDerivativeAtPosition(ForceField::InterpolationGridType::LennardJones, system.forceField, system.simulationBox, posB, typeB, frameworkAtoms);
...
TEST(third_derivative_inter_lennard_jones, Test_third_derivative_fractional_methane_in_CHA_triclinic_1x1x1)
{
  ...
  {
    double3 s = system.simulationBox.inverseCell * posB;
    auto [e, d1, d2, reference_cartesian] = Interactions::calculateTricubicDerivativeAtPosition(ForceField::InterpolationGridType::LennardJones, system.forceField, system.simulationBox, posB, typeB, frameworkAtoms);

    double reference_fractional = 
      system.simulationBox.cell.ax * system.simulationBox.cell.bx * system.simulationBox.cell.cx * reference_cartesian[0][0][0] +
      system.simulationBox.cell.ax * system.simulationBox.cell.bx * system.simulationBox.cell.cy * reference_cartesian[0][0][1] +
      system.simulationBox.cell.ax * system.simulationBox.cell.bx * system.simulationBox.cell.cz * reference_cartesian[0][0][2] +
      system.simulationBox.cell.ax * system.simulationBox.cell.by * system.simulationBox.cell.cx * reference_cartesian[0][1][0] +
      system.simulationBox.cell.ax * system.simulationBox.cell.by * system.simulationBox.cell.cy * reference_cartesian[0][1][1] +
      system.simulationBox.cell.ax * system.simulationBox.cell.by * system.simulationBox.cell.cz * reference_cartesian[0][2][1];
