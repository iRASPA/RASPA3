module;

#define CL_TARGET_OPENCL_VERSION 120
#ifdef __APPLE__
#include <OpenCL/cl.h>
#elif _WIN32
#include <CL/cl.h>
#else
#include <CL/opencl.h>
#endif

export module mc_opencl_pore_size_distribution;

import std;

import sampled_structure;

// The sampled pore-size distribution on the GPU. See `mc_pore_size_distribution` for what is being
// estimated: the inner loop over trial sphere centres is what the device does, one centre per work item,
// each reporting the radius it found to a single atomic maximum.
export struct MC_OpenCL_PoreSizeDistribution
{
  cl_program poreSizeDistributionProgram;
  cl_kernel poreSizeDistributionKernel;
  static const char* poreSizeDistributionKernelSource;
  size_t poreSizeDistributionWorkGroupSize;

  std::size_t numberOfBins;
  std::vector<double> histogram;
  std::vector<double> histogram_cummulative;

  double seconds{0.0};

  MC_OpenCL_PoreSizeDistribution(std::size_t numberOfBins);
  ~MC_OpenCL_PoreSizeDistribution();

  void run(const SampledStructure& structure, std::optional<std::size_t> numberOfIterations,
           std::optional<std::size_t> numberOfInnerSteps, std::optional<double> maximumRange);
};
