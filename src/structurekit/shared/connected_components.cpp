...
module grid_connected_components;
...
GridComponents GridComponents::compute(uint3 gridSize, std::span<const float> openness, double lowestOpenness)
{
  std::chrono::steady_clock::time_point time_begin = std::chrono::steady_clock::now();

  GridComponents components;
  components.lowestOpenness = lowestOpenness;
  ...
  const float threshold = static_cast<float>(lowestOpenness);
  ...
  for (std::size_t start = 0; start < labelVoxel.size(); ++start)
  {
...

    {
      ...
      pore.largestOpenness = static_cast<double>(openness[static_cast<std::size_t>(v)]);
    }
    ...
    while (!stack.empty())
    {
      ...
      const double room = static_cast<double>(openness[static_cast<std::size_t>(voxel)]);
      const std::int32_t k = voxel / (nx * ny);
      const std::int32_t j = (voxel / nx) % ny;
      const std::int32_t i = voxel % nx;
      if (room > pore.largestOpenness)
      {
        pore.largestOpenness = room;
