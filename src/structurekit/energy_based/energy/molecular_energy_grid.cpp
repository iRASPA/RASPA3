                                                    ...
                                                    const ElectrostaticPotentialGrid *potential)
{
  ...
  // A plane of the grid is settled by the atoms and the orientations, neither of which anything here writes to,
  // and lands in its own stretch of the two fields, so the planes are independent work.
  forEachIndex(gridSize.z, workersAvailable(),
               [&](std::size_t, std::size_t iz)
               {
                 for (std::size_t iy = 0; iy < gridSize.y; ++iy)
                 {
                   ...
                   {
                     // Endpoint-exclusive sampling, as in the other fields of this route, so that what is read off one
                     // may be read off another. The point is the molecule's centre of mass; its sites hang off it.
                     ...
                     // The average of exp(-U/kT) over orientations is accumulated as it goes, rescaled whenever a
                     // deeper orientation turns up. Taken plainly the exponential would overflow at a strong site long
                     // before the physics did; carried this way every term is at most one and the least is harmless.
                         ...
                         for (std::size_t iatom = 0; iatom < numberOfAtoms; ++iatom)
                         {
                           ...
                           for (std::int32_t a = -shells.x; a <= shells.x; ++a)
                           {
                             for (std::int32_t b = -shells.y; b <= shells.y; ++b)
                             {
                               ...
                               {
                                 double3 t = ds + double3(static_cast<double>(a), static_cast<double>(b),
                                                          static_cast<double>(c));

                                 // A fractional difference is at least |t_k| w_k long whatever the shape of the cell,
                                 // so an image beyond the cutoff is thrown out here before it is built.
                                 double far = std::max(
                                     {std::abs(t.x) * widths.x, std::abs(t.y) * widths.y, std::abs(t.z) * widths.z});
                                 ...
                                 // separation off zero keeps the sum a number; such an orientation is buried far above
                                 // the ceiling and contributes nothing to the average either way.
                     ...
                     std::size_t index = (iz * gridSize.y + iy) * gridSize.x + ix;
                     ...
                     // measured from the least, so the logarithm is never positive and the free energy never falls
                     // below the minimum: the gap between them is what the molecule pays for having to be turned a
                     // particular way.
                                                                 ...
                                                                 const ElectrostaticPotentialGrid *potential)
{
  ...
  // atoms can be gone through, so it is gathered first and the atoms are visited afterwards. One of these per
  // worker rather than one per point: no two planes are in the hands of the same worker at once, so a worker
  // may keep reusing its own.
  const std::size_t workers = workersAvailable();
  std::vector<std::vector<double>> weightOfWorker(workers, std::vector<double>(numberOfDirections, 0.0));

  forEachIndex(gridSize.z, workers,
               [&](std::size_t worker, std::size_t iz)
               {
                 std::vector<double> &weight = weightOfWorker[worker];
                 ...
                 for (std::size_t iy = 0; iy < gridSize.y; ++iy)
                 {
                   for (std::size_t ix = 0; ix < gridSize.x; ++ix)
                   {
                     ...
                     // out equal, which is the right answer there: no orientation is preferred because none is
                     // possible.
