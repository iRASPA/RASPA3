...
{
  double3 delta = unitCell.applyPeriodicBoundaryConditions(a - b);
  return delta.length();
}
                                                   ...
                                                   std::size_t numberOfSamples)
{
  ...
  const UnitCell& unitCell = accessibility.unitCell;
  std::vector<BlockingSphere> spheres;
  ...
  for (std::size_t s = 0; s < samples; ++s)
  {
    ...
    {
      point = unitCell.cell * double3(random.uniform(), random.uniform(), random.uniform());
  ...
  // single sample.
  double resolution = std::cbrt(unitCell.volume / static_cast<double>(samples));
  ...
  double channelMargin = accessiblePoints.empty()
                             ? 0.0
                             : std::cbrt(unitCell.volume / static_cast<double>(accessiblePoints.size()));
  ...
  for (auto& [poreId, points] : pocketPoints)
  {
    ...
    for (std::size_t i = 0; i < points.size(); ++i)
    {
      ...
      {
        if (covered[j] == 0)
          furthestPocket = std::max(furthestPocket, periodicDistance(unitCell, center, points[j].position));
      }

      double closestChannel = std::numeric_limits<double>::max();
      for (const double3& p : accessiblePoints)
        closestChannel = std::min(closestChannel, periodicDistance(unitCell, center, p));
      ...
      spheres.push_back(BlockingSphere{double3::fract(unitCell.inverseCell * center), radius});

      for (std::size_t j = i; j < points.size(); ++j)
      {
        if (covered[j] == 0 && periodicDistance(unitCell, center, points[j].position) < radius) covered[j] = 1;
      }
    }
  }
...
{
  writeBlockFile(frameworkName, spheres);
