...
VolumeSample sampleAccessibleVolume(const PoreAccessibility& accessibility, std::size_t numberOfSamples)
{
  ...
  for (std::size_t s = 0; s < samples; ++s)
  {
    ...
    for (std::size_t attempt = 0; attempt < resampleLimit; ++attempt)
    {
      ...
      classification = accessibility.classify(accessibility.unitCell.cell * fractional);
...
void VoronoiAccessibleVolume::run(const PairInteractions& interactions, const Crystal& framework,
