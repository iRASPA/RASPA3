...
static double3 perpendicularWidthsOf(const UnitCell& unitCell)
{
...
PoreWindow PoreWindow::measure(const VoronoiNetwork& network, const double3& position, const double3& normal)
{
  ...
  const double3x3 cell = network.unitCell.cell;
  const double3x3 inverseCell = network.unitCell.inverseCell;
  const std::vector<double>& radii = network.atomRadii;
  double3 perpendicularWidths = perpendicularWidthsOf(network.unitCell);
