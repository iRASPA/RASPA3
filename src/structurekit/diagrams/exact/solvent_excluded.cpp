  ...
  for (std::size_t i = 0; i < accessibility.atomPositions.size(); ++i)
  {
    ...
    for (const SphereCircle& circle : boundary.circles)
    {
      ...
      const double3 neighbourCentre =
          ...
          accessibility.unitCell.cell * double3(static_cast<double>(circle.neighbourImage.x),
  ...
  for (std::size_t i = 0; i < accessibility.atomPositions.size(); ++i)
  {
    ...
    for (const SphereCircle& circle : boundary.circles)
    {
      for (double angle : circle.cornerAngles)
      {
        ...
        double3 fractional = double3::fract(unitCell.inverseCell * position);
        double3 folded = unitCell.cell * fractional;
        ...
        for (int ox = -1; ox <= 1 && !seen; ++ox)
        {
          for (int oy = -1; oy <= 1 && !seen; ++oy)
          {
            for (int oz = -1; oz <= 1 && !seen; ++oz)
            {
              ...
              {
                double3 delta = unitCell.applyPeriodicBoundaryConditions(folded - wrapped[other]);
...
struct VertexGrid
{
  const UnitCell* unitCell{nullptr};
  int3 gridSize{1, 1, 1};
      ...
      for (int ox = -k; ox <= k; ++ox)
      {
        for (int oy = -k; oy <= k; ++oy)
        {
          for (int oz = -k; oz <= k; ++oz)
          {
            ...
            double3 imageShift = unitCell->cell * double3(static_cast<double>(lx), static_cast<double>(ly),
...
{
  VertexGrid grid;
  grid.unitCell = &accessibility.unitCell;
  ...
  {
    double3 fractional = double3::fract(accessibility.unitCell.inverseCell * vertices[i].position);
    grid.folded.push_back(accessibility.unitCell.cell * fractional);
                                                ...
                                                const MeasuredPatches& patches, std::size_t subdivisions)
{
  ...
  result.poreVolume = accessibility.unitCell.volume - result.excludedVolume;
