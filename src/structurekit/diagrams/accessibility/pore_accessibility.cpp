...
PoreAccessibility PoreAccessibility::create(const UnitCell& unitCell,
                                                  ...
                                                  const std::vector<double>& radii, double probeRadius)
{
  ...
  return createFromNetwork(VoronoiNetwork::create(unitCell, fractionalPositions, inflatedRadii), Metric::Power,
                           probeRadius);
}
...
PoreAccessibility PoreAccessibility::createFromNetwork(VoronoiNetwork network, Metric metric, double probeRadius)
{
  ...
  const UnitCell unitCell = network.unitCell;
  accessibility.unitCell = unitCell;
  ...
  {
    accessibility.atomPositions.push_back(unitCell.cell * fractional);
  }
  ...
  for (std::size_t i = 0; i < accessibility.network.nodes.size(); ++i)
  {
    ...
    double3 fractional = double3::fract(unitCell.inverseCell * accessibility.network.nodes[i].position);
...
PointClassification PoreAccessibility::classify(const double3& point) const
{
    ...
    for (int ox = -k; ox <= k; ++ox)
    {
      for (int oy = -k; oy <= k; ++oy)
      {
        for (int oz = -k; oz <= k; ++oz)
        {
          ...
          double3 imageShift =
              unitCell.cell * double3(static_cast<double>(lx), static_cast<double>(ly), static_cast<double>(lz)) -
              wrappedPoint;
  ...
  for (std::size_t nodeIndex = 0; nodeIndex < network.nodes.size(); ++nodeIndex)
  {
    ...
    double3 delta = unitCell.applyPeriodicBoundaryConditions(wrappedPoint - network.nodes[nodeIndex].position);
...
int3 PoreAccessibility::nearestLift(const double3& point, std::int32_t poreId) const
{
  ...
  for (std::size_t nodeIndex : pore.nodeIndices)
  {
    ...
    double3 assembled = network.nodes[nodeIndex].position +
                        unitCell.cell * double3(static_cast<double>(offset.x), static_cast<double>(offset.y),
                                                     static_cast<double>(offset.z));
    double3 fractional = unitCell.inverseCell * (point - assembled);
    ...
    double3 shift = unitCell.cell * double3(static_cast<double>(translation.x),
