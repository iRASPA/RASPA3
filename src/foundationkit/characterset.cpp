...
CharacterSet::CharacterSet(std::string chars)
{
	_string = chars;
}
...
{
	return CharacterSet(std::string("\x0A", 1));
}

CharacterSet CharacterSet::newlineCharacterSet()
{
	return CharacterSet(std::string("\x0A\x0B\x0C\x0D\x85", 5));
}
...
{
	return CharacterSet(std::string("\x20\x09\xA0", 3));
}
