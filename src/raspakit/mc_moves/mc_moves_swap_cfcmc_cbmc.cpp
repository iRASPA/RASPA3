                                                                               ...
                                                                               bool deletionDisabled)
{
  ...
  double maxChange = system.components[selectedComponent].mc_moves_statistics.swapMove_CFCMC_CBMC.maxChange[2];
  ...
  if (selectedNewBin >= std::make_signed_t<std::size_t>(lambda.numberOfBins))  // Insertion move
  {
    ...
    // computed (2) Biased: a new fractional molecule is grown with lambda_new = epsilon
