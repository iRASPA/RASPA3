...
{
  const Move::Types move = Move::Types::ReactionConventionalCFCMCCBMC;
  ...
  RunningEnergy scalingEnergyDifference{};
  ...
  else
  {
    ...
    scalingEnergyDifference = scalingDifference.value();
    energyDifference = scalingEnergyDifference;
  ...
  else
  {
    ...
    rosenbluthRatio *= std::exp(-system.beta * tailEnergyDifference.potentialEnergy());
    acceptanceProbability = correctionFactorEwald * rosenbluthRatio *
                            std::exp(-system.beta * scalingEnergyDifference.potentialEnergy()) *
                            std::exp(equilibriumTerm + biasNew - biasOld);
  }
