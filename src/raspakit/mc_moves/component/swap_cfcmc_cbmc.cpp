                                                                               ...
                                                                               bool deletionDisabled)
{
  ...
  if (selectedNewBin >= std::make_signed_t<std::size_t>(lambda.numberOfSamplePoints))  // Insertion move
  {
    ...
    RunningEnergy EwaldEnergyDifference = timed(
    ...
    std::optional<CBMC::GrowResult> growData =
        timed(system, component, move, Move::Timing::InsertionNonEwald,
              [&]
              {
    ...
    RunningEnergy tailEnergyDifferenceGrow =
        timed(system, component, move, Move::Timing::InsertionTail,
              [&]
              {
    ...
    // molecules and rescales its own polarization coupling from scalingCoulomb(lambda_old) to 1; (step 2) growing the
    // new fractional molecule adds its own (lambda-scaled) polarization energy and again changes the field on the
    // other molecules (its inter-molecular energy is already accounted for through CBMC).
    ...
    if (system.forceField.computePolarization && !system.forceField.omitInterPolarization)
    {
      ...
      polarizationDifference += Interactions::computePolarizationEnergyDifference(
                                    system.forceField, grownElectricField, {}, growData->atoms, {}) +
  ...
  else if (selectedNewBin < 0)  // Deletion move
  {
    ...
    if (system.numberOfIntegerMoleculesPerComponent[selectedComponent] > 0)
    {
      ...
      // molecules changes because the removed molecule disappears and because the newly chosen fractional molecule
      // is scaled down.
      std::vector<double3> electricFieldNeighborDelta;
      RunningEnergy polarizationDifference;
      if (system.forceField.computePolarization)
      {
        ...
        removedSelf += Interactions::computePolarizationEnergyDifference(
  ...
  else  // Lambda-move
  {
    ...
    [[maybe_unused]] RunningEnergy tailEnergyDifference = timed(
        system, component, move, Move::Timing::LambdaTail,
        [&]
        {
          return Interactions::computeInterMolecularTailEnergyDifferenceAggregated(
