...
export struct PartialMolarPropertiesTerms
{
  ...
  inline PartialMolarPropertiesData zeroCompositeProperty [[nodiscard]] () const
  {
    return PartialMolarPropertiesData(size);
  }
...
/**
 * \brief Block-averaged partial molar internal energy and partial molar volume.
 *
 * A composite (fluctuation-derived) property: PartialMolarPropertiesTerms are accumulated per block
 * and combined through the matrix fluctuation formula. All accumulation, averaging and error
 * estimation is inherited from BlockAverage; only the partial-molar-specific output
 * formatting lives here.
 */
export struct PropertyPartialMolarProperties : BlockAverage<PartialMolarPropertiesTerms>
{
  PropertyPartialMolarProperties() = default;
  PropertyPartialMolarProperties(std::size_t numberOfBlocks, std::size_t numberOfComponents)
      : BlockAverage<PartialMolarPropertiesTerms>(numberOfBlocks, PartialMolarPropertiesTerms(numberOfComponents))
  {
  }
  ...
  {
    BlockAverage<PartialMolarPropertiesTerms>::resize(PartialMolarPropertiesTerms(newNumberOfComponents));
  }
