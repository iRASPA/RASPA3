...
std::string PropertyGibbsWidom::writeAveragesChemicalPotentialStatistics(double beta,
...
Archive<std::ofstream> &operator<<(Archive<std::ofstream> &archive, const PropertyGibbsWidom &w)
{
...
Archive<std::ifstream> &operator>>(Archive<std::ifstream> &archive, PropertyGibbsWidom &w)
{
  ...
  if (versionNumber > w.versionNumber)
  {
    ...
    throw std::runtime_error(std::format("Invalid version reading 'PropertyGibbsWidom' at line {} in file {}\n",
  ...
  {
    throw std::runtime_error(std::format("PropertyGibbsWidom: Error in binary restart\n"));
  }
#endif
