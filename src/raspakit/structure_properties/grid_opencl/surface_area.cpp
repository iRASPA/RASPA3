...
void GridSurfaceArea::run(const ForceField &forceField, const Framework &framework, std::string probePseudoAtom,
                          uint3 gridSize)
{
                          ...
                          const ClearanceGrid &grid)
{
  ...
  this->atomSurfaceArea.assign(framework.unitCellAtoms.size(), 0.0);
  ...
  const double toGravimetric = Units::Angstrom * Units::Angstrom * Units::AvogadroConstant / framework.unitCellMass;
  ...
  for (std::size_t i = 0; i < this->atomSurfaceArea.size(); ++i)
  {
    ...
    std::size_t type = static_cast<std::size_t>(framework.unitCellAtoms[i].type);
    std::print(myfile, "Atom: {:6} {:<20} {:13.5f}\n", i, forceField.pseudoAtoms[type].name,
