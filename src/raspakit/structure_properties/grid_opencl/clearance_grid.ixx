...
export struct ClearanceGrid
{
  ...
  // The radii the field was built from, in the order of `framework.unitCellAtoms`.
  ...
  void writeTessellation(const Framework &framework) const;

  // Builds the field on the GPU. Throws if no OpenCL device was found.
  static ClearanceGrid compute(const ForceField &forceField, const Framework &framework, uint3 gridSize);
