...
{
  return this->simulationBox.cell * this->fractionalPosition(i, j, k);
}
...
double ClearanceGrid::voxelVolume() const
{
  ...
  return this->simulationBox.volume / static_cast<double>(this->clearance.size());
}
...
void ClearanceGrid::writeTessellation(const Framework &framework) const
{
  ...
  std::vector<std::size_t> voxelsPerAtom(framework.unitCellAtoms.size(), 0);
  ...
  for (std::size_t atom = 0; atom < voxelsPerAtom.size(); ++atom)
  {
    ...
    std::print(myfile, "Atom: {:6} {:11.5f} {:9} {:13.5f} {:12.8f}\n", atom,
               (atom < this->atomRadii.size()) ? this->atomRadii[atom] : 0.0, voxelsPerAtom[atom], share,
               share / this->simulationBox.volume);
  }
...
{
  double3 a = this->simulationBox.cell[0];
  double3 b = this->simulationBox.cell[1];
  double3 c = this->simulationBox.cell[2];
...
ClearanceGrid ClearanceGrid::compute(const ForceField &forceField, const Framework &framework, uint3 gridSize)
{
  ...
  grid.simulationBox = framework.simulationBox;
  ...
  grid.atomRadii.reserve(framework.unitCellAtoms.size());
  for (const Atom &atom : framework.unitCellAtoms)
  {
    std::size_t type = static_cast<std::size_t>(atom.type);
    grid.atomRadii.push_back(0.5 * forceField(type, type).sizeParameter());
  }
  ...
  double3x3 cell = framework.simulationBox.cell;
