...
void MC_OpenCL_SurfaceArea::run(const ForceField &forceField,
                                const Framework &framework, 
                                double wellDepthFactor,
                                std::string probePseudoAtom,
                                std::size_t numberOfSlices) const
{
  cl_int err;
  ...
  cl_int number_of_slices = cl_int(numberOfSlices);
  ...
  err |= clSetKernelArg(surfaceAreaKernel, 4, sizeof(cl_int), &number_of_slices);
                                 ...
                                 const float4 inverse_cellc)
{
  ...
  if(iatom < numberOfAtoms)
  {
    ...
    for(int stackNumber = 0; stackNumber <= numberOfSlices; ++stackNumber)
    {
      for(int sliceNumber = 0; sliceNumber < numberOfSlices; ++sliceNumber)
      {
        float theta = float(sliceNumber) * 2.0 * M_PI / float(numberOfSlices);

        float u = float(stackNumber) / float(numberOfSlices);
