...
BondPotential::BondPotential(std::array<size_t, 2> identifiers, BondType type, std::vector<double> vector_parameters) :
      identifiers(identifiers), type(type)
{
  for(size_t i = 0; i < std::min(parameters.size(), maximumNumberOfBondParameters); ++i)
  {
  ...
  switch (type)
  {
    ...
    case BondType::Buckingham:
      return std::format("{} - {} : BUCKINGHAM p_0/k_B={:g} [K], p_1={:g} [Å^-1], p_2/k_B={:g} [K Å^6]\n", identifiers[0], identifiers[1],
                         parameters[0] * Units::EnergyToKelvin, parameters[1], parameters[2] * Units::EnergyToKelvin);
    case BondType::RestrainedHarmonic:
    ...
    case BondType::CFF_Quartic:
      return std::format("{} - {} : CFF_QUARTIC p_0/k_B={:g} [K/Å^2], p_1={:g} [Å], p_2={:g} [K/Å^3], p_3={:g} [K/Å^4]\n", 
                         identifiers[0], identifiers[1], parameters[0] * Units::EnergyToKelvin, parameters[1],
                         parameters[2] * Units::EnergyToKelvin, parameters[3] * Units::EnergyToKelvin);
    case BondType::MM3:
      return std::format("{} - {} : MM3 p_0/k_B={:g} [mdyne/Å molecule], p_1={:g} [Å]\n", identifiers[0], identifiers[1],
                         parameters[0] / (71.94 * Units::KCalPerMolToEnergy), parameters[1]);
    default:
      std::unreachable();
  }
}
