...
BondBendPotential::BondBendPotential(std::array<size_t, 4> identifiers, BondBendType type, std::vector<double> vector_parameters) :
      identifiers(identifiers), type(type)
{
  for(size_t i = 0; i < std::min(parameters.size(), maximumNumberOfBondBendParameters); ++i)
  {
