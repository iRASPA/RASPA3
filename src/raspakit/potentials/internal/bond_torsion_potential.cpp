...
BondTorsionPotential::BondTorsionPotential(std::array<size_t, 4> identifiers, BondTorsionType type, std::vector<double> vector_parameters) :
      identifiers(identifiers), type(type)
{
  for(size_t i = 0; i < std::min(parameters.size(), maximumNumberOfBondTorsionParameters); ++i)
  {
