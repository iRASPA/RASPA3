...
BendPotential::BendPotential(std::array<size_t, 3> identifiers, BendType type, std::vector<double> vector_parameters) :
      identifiers(identifiers), type(type)
{
  for(size_t i = 0; i < std::min(parameters.size(), maximumNumberOfBendParameters); ++i)
  {
