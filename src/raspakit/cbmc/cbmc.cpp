...
[[nodiscard]] std::optional<ChainData> CBMC::growMoleculeSwapInsertion(
    ...
    double beta, Component::GrowType growType, double cutOff, double cutOffCoulomb, size_t selectedComponent,
    size_t selectedMolecule, double scaling, size_t groupId, size_t numberOfTrialDirections) noexcept
{
  switch (growType)
  {
    default:
      return CBMC::growRigidMoleculeSwapInsertion(
          random, hasExternalField, components, forceField, simulationBox, frameworkAtoms, moleculeAtoms, beta, cutOff,
          cutOffCoulomb, selectedComponent, selectedMolecule, scaling, groupId, numberOfTrialDirections);
  }
}
...
{
  return CBMC::growRigidMoleculeReinsertion(
      random, hasExternalField, components, forceField, simulationBox, frameworkAtoms, moleculeAtoms, beta, cutOff,
      cutOffCoulomb, selectedComponent, selectedMolecule, molecule, molecule_atoms, numberOfTrialDirections);
}
    ...
    Molecule &molecule, std::span<Atom> molecule_atoms, double storedR, size_t numberOfTrialDirections) noexcept
{
  return CBMC::retraceRigidMoleculeReinsertion(
      random, hasExternalField, components, forceField, simulationBox, frameworkAtoms, moleculeAtoms, beta, cutOff,
      cutOffCoulomb, selectedComponent, selectedMolecule, molecule, molecule_atoms, storedR, numberOfTrialDirections);
}
    ...
    std::span<Atom> molecule, double scaling, size_t numberOfTrialDirections) noexcept
{
  return CBMC::retraceRigidMoleculeSwapDeletion(
      random, hasExternalField, components, forceField, simulationBox, frameworkAtoms, moleculeAtoms, beta, cutOff,
      cutOffCoulomb, selectedComponent, selectedMolecule, molecule, scaling, numberOfTrialDirections);
}
