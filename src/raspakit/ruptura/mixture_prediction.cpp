...
std::vector<std::size_t> sortedIndices(const std::span<const Component> &v)
{
      ...
      temperature(system.temperature)
{
  ...
  {
    for (std::size_t i = 0; i < maxIsothermTerms; ++i)
    {
                                                                       ...
                                                                       double *cachedP0, double *cachedPsi)
{
  ...
  std::size_t numberOfIASTSteps = 0;
  ...
  if (cachedPsi[0] > 0.0)
  {
    for (std::size_t i = 0; i < Nsorted; ++i)
    {
  ...
  {
    // compute G
    for (std::size_t i = 0; i < Nsorted - 1; ++i)
    {
    ...
    // trick to loop downward from Nsorted - 2 to and including zero (still using std::size_t as index)
    for (std::size_t i = Nsorted - 1; i-- != 0;)
    {
                                                                        ...
                                                                        double *cachedPsi)
{
  ...
  std::pair<std::size_t, std::size_t> acc;
  for (std::size_t i = 0; i < maxIsothermTerms; ++i)
  {
                                                                               ...
                                                                               std::vector<double> &Ni)
{
  ...
  alpha2[Ncomp - 1] = 1.0 + sortedComponents[Ncomp - 1].get().isotherm.sites[0].parameters[1] *
                                Yi[sortedComponents[Ncomp - 1].get().componentId] * P;
  for (std::size_t i = Ncomp - 2; i > 0; i--)
  {
  ...
  for (std::size_t i = 0; i < Ncomp; ++i)
  {
    std::size_t index = sortedComponents[i].get().componentId;
    ...
    std::size_t site, const std::vector<double> &Yi, const double &P, std::vector<double> &Xi, std::vector<double> &Ni)
{
  ...
  for (std::size_t i = 0; i < Ncomp; ++i)
  {
    std::size_t index = segregatedSortedComponents[site][i].componentId;
...
void MixturePrediction::run(std::ostream &stream)
{
  ...
  std::filesystem::create_directory(std::format("MixturePrediction/System_{}", 0));
  std::vector<std::ofstream> streams;
  for (std::size_t i = 0; i < Ncomp; i++)
  {
    std::string fileName = std::format("MixturePrediction/System_{}/component_{}_{}.txt", 0,
...
void MixturePrediction::createPureComponentsPlotScript()
{
  std::filesystem::create_directory("MixturePrediction");
  std::filesystem::create_directory(std::format("MixturePrediction/System_{}", 0));
  std::ofstream stream(std::format("MixturePrediction/System_{}/plot_pure_components", 0));
...
void MixturePrediction::createMixturePlotScript()
{
  std::filesystem::create_directory("MixturePrediction");
  std::filesystem::create_directory(std::format("MixturePrediction/System_{}", 0));
  std::ofstream stream(std::format("MixturePrediction/System_{}/plot_mixture", 0));
...
void MixturePrediction::createMixtureAdsorbedMolFractionPlotScript()
{
  std::filesystem::create_directory("MixturePrediction");
  std::filesystem::create_directory(std::format("MixturePrediction/System_{}", 0));
  std::ofstream stream(std::format("MixturePrediction/System_{}/plot_mixture_mol_fractions", 0));
...
void MixturePrediction::createPlotScript()
{
  ...
  std::filesystem::create_directory("MixturePrediction");
  std::filesystem::create_directory(std::format("MixturePrediction/System_{}", 0));
  std::ofstream stream_graphs(std::format("MixturePrediction/System_{}/make_graphs.bat", 0));
  ...
  std::filesystem::create_directory("MixturePrediction");
  std::filesystem::create_directory(std::format("MixturePrediction/System_{}", 0));
  std::ofstream stream_graphs(std::format("MixturePrediction/System_{}/make_graphs", 0));
  ...
  std::filesystem::path path{std::format("MixturePrediction/System_{}/make_graphs", 0)};
  std::filesystem::permissions(path, std::filesystem::perms::owner_exec, std::filesystem::perm_options::add);
#endif
}
                                         ...
                                         double cachedP0[])
{
  ...
  for (std::size_t i = 0; i < Ncomp; ++i) std::cout << "cachedP0: " << cachedP0[i] << std::endl;
  for (std::size_t i = 0; i < Ncomp; ++i)
  {
