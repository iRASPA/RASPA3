...
std::string OpenCL::printBestOpenCLDevice()
{
  ...
  if(clDeviceId.has_value())
  {
    ...
    err = clGetDeviceInfo(clDeviceId.value(), CL_DEVICE_MAX_WORK_ITEM_DIMENSIONS, sizeof(workitem_dims), &workitem_dims, NULL);
    if (err == CL_SUCCESS) stream << "CL_DEVICE_MAX_WORK_ITEM_DIMENSIONS:" << workitem_dims << "\n";

    // CL_DEVICE_MAX_WORK_ITEM_SIZES
    size_t workitem_size[3];
    err = clGetDeviceInfo(clDeviceId.value(), CL_DEVICE_MAX_WORK_ITEM_SIZES, sizeof(workitem_size), &workitem_size, NULL);
    if (err == CL_SUCCESS) stream << "CL_DEVICE_MAX_WORK_ITEM_SIZES: " <<  workitem_size[0] << "," << workitem_size[1] << "," << workitem_size[2] << "\n";

    // CL_DEVICE_MAX_WORK_GROUP_SIZE
    size_t workgroup_size;
    err = clGetDeviceInfo(clDeviceId.value(), CL_DEVICE_MAX_WORK_GROUP_SIZE, sizeof(workgroup_size), &workgroup_size, NULL);
    if (err == CL_SUCCESS) stream << "CL_DEVICE_MAX_WORK_GROUP_SIZE: " << workgroup_size << "\n";

    // CL_DEVICE_MAX_CLOCK_FREQUENCY
    cl_uint clock_frequency;
    err = clGetDeviceInfo(clDeviceId.value(), CL_DEVICE_MAX_CLOCK_FREQUENCY, sizeof(clock_frequency), &clock_frequency, NULL);
    ...
    err = clGetDeviceInfo(clDeviceId.value(), CL_DEVICE_MAX_MEM_ALLOC_SIZE, sizeof(max_mem_alloc_size), &max_mem_alloc_size, NULL);
    if (err == CL_SUCCESS) stream << std::format("CL_DEVICE_MAX_MEM_ALLOC_SIZE: {} MByte\n", static_cast<unsigned int>(max_mem_alloc_size / (1024 * 1024)));

    // CL_DEVICE_GLOBAL_MEM_SIZE
    cl_ulong mem_size;
    err = clGetDeviceInfo(clDeviceId.value(), CL_DEVICE_GLOBAL_MEM_SIZE, sizeof(mem_size), &mem_size, NULL);
    if (err == CL_SUCCESS) stream << std::format("CL_DEVICE_GLOBAL_MEM_SIZE: {} MByte\n", static_cast<unsigned int>(mem_size / (1024 * 1024)));

    // CL_DEVICE_ERROR_CORRECTION_SUPPORT
    cl_bool error_correction_support;
    err = clGetDeviceInfo(clDeviceId.value(), CL_DEVICE_ERROR_CORRECTION_SUPPORT, sizeof(error_correction_support), &error_correction_support, NULL);
    if (err == CL_SUCCESS) stream << std::format("CL_DEVICE_ERROR_CORRECTION_SUPPORT: {}\n", error_correction_support == CL_TRUE ? "yes" : "no");

    // CL_DEVICE_LOCAL_MEM_TYPE
    cl_device_local_mem_type local_mem_type;
    err = clGetDeviceInfo(clDeviceId.value(), CL_DEVICE_LOCAL_MEM_TYPE, sizeof(local_mem_type), &local_mem_type, NULL);
    if (err == CL_SUCCESS) stream << std::format("CL_DEVICE_LOCAL_MEM_TYPE: {}\n", local_mem_type == 1 ? "local" : "global");

    // CL_DEVICE_LOCAL_MEM_SIZE
    err = clGetDeviceInfo(clDeviceId.value(), CL_DEVICE_LOCAL_MEM_SIZE, sizeof(mem_size), &mem_size, NULL);
    if (err == CL_SUCCESS) stream << std::format("CL_DEVICE_LOCAL_MEM_SIZE: {} KByte\n", static_cast<unsigned int>(mem_size / 1024));

    // CL_DEVICE_MAX_CONSTANT_BUFFER_SIZE
    err = clGetDeviceInfo(clDeviceId.value(), CL_DEVICE_MAX_CONSTANT_BUFFER_SIZE, sizeof(mem_size), &mem_size, NULL);
    if (err == CL_SUCCESS) stream << std::format("CL_DEVICE_MAX_CONSTANT_BUFFER_SIZE: {} KByte\n", static_cast<unsigned int>(mem_size / 1024));
    ...
    err = clGetDeviceInfo(clDeviceId.value(), CL_DEVICE_MAX_READ_IMAGE_ARGS, sizeof(max_read_image_args), &max_read_image_args, NULL);
    if (err == CL_SUCCESS) stream << std::format("CL_DEVICE_MAX_READ_IMAGE_ARGS: {}\n", max_read_image_args);

    // CL_DEVICE_MAX_WRITE_IMAGE_ARGS
    cl_uint max_write_image_args;
    err = clGetDeviceInfo(clDeviceId.value(), CL_DEVICE_MAX_WRITE_IMAGE_ARGS, sizeof(max_write_image_args), &max_write_image_args, NULL);
