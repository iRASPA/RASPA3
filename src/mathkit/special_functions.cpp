...
int areEqual (double a, double b)
{
  ...
  if ( areClose(a, b, std::numeric_limits<double>::epsilon()))
    return 1;
...
double hypergeometric2F1(double a, double b, double c, double z)
{
  ...
  for (k = 0; k <= kMax; k++)
  {
    ...
    e_k1 = (lambda * z * (-a * b * xi * d_k + (static_cast<double>(k) + c) * e_k));
		t    =  d_k * (-static_cast<double>(k) / (-1.0 + z) - (b * a - (mu + static_cast<double>(k)) * 
            static_cast<double>(k)) / (2.0 * static_cast<double>(k) + c) * xi) + e_k;
    f_k1 = (f_k + t);
    if (areEqual(f_k1, f_k) == 1 && maxNORM(t) <= std::numeric_limits<double>::epsilon() && 3 < k)
      break;
    d_k = (d_k1);
    e_k = (e_k1);
    f_k = (f_k1);
