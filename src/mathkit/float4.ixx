...
export union float4
{
  ...
  {
    return float4(a.y * b.z - a.z * b.y, -a.x * b.z + a.z * b.x, a.x * b.y - a.y * b.x, 0.0);
  }
...
{
  return float4(a.x * static_cast<float>(b), a.y * static_cast<float>(b), a.z * static_cast<float>(b),
                a.w * static_cast<float>(b));
}

inline float4 operator*(const double& a, float4 const& b)
{
  return float4(static_cast<float>(a) * b.x, static_cast<float>(a) * b.y, static_cast<float>(a) * b.z,
                static_cast<float>(a) * b.w);
}

export inline float4 operator*(const float4& a, const float& b) { return float4(a.x * b, a.y * b, a.z * b, a.w * b); }

export inline float4 operator*(const float& a, float4 const& b) { return float4(a * b.x, a * b.y, a * b.z, a * b.w); }
