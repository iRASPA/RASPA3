...
RingMatrix::RingMatrix(size_t rows, size_t columns, int initialValue) : _rows(rows), _columns(columns), _grid(rows* columns)
{
...
RingMatrix::RingMatrix(size_t rows, size_t columns, std::vector<std::vector<int>> data) : _rows(rows), _columns(columns), _grid(rows* columns)
{
...
RingMatrix RingMatrix::zeros(size_t rows, size_t columns)
{
    return RingMatrix(rows, columns, 0);
}

RingMatrix RingMatrix::ones(size_t rows, size_t columns)
{
    return RingMatrix(rows, columns, 1);
}
...
std::tuple<RingMatrix, RingMatrix, std::vector<size_t>>  RingMatrix::HermiteNormalForm()
{
        ...
        if (found)
        {
            ...
            for (size_t i = 0; i < QQ._rows; i++)
            {
                if (i != r - 1 && i != r)
                {
                    ...
                    {
                        QQ(i, l) = QQ(i, l) + std::get<0>(columnReduction)(i, r - 1) * QQ(r - 1, l) + std::get<0>(columnReduction)(i, r) * QQ(r, l);
                    }
                }
            }
    ...
    rp = std::vector<size_t>(rp.begin() + 1, rp.begin() + static_cast<std::vector<size_t>::difference_type>(r));  //rp = Array(rp[1..<r])
...
std::tuple<RingMatrix, RingMatrix, RingMatrix> RingMatrix::ColumnReduction(RingMatrix A1, size_t col_1, size_t col_2, size_t row_start)
{
    RingMatrix A = A1;
    size_t n = A._rows;
    size_t m = A._columns;
    ...
    std::tuple<int, int, int> extendedGCD = Ring::extendedGreatestCommonDivisor(A(row_start, col_1), A(row_start + 1, col_1));
...
std::pair<std::vector<int>, std::vector<int>> RingMatrix::Conditioning(RingMatrix A, size_t col_1, size_t col_2, size_t row_start)
{
